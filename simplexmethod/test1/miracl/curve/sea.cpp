...
//
// To obtain the full source code, plus the Windows executables, first look at
...
#include <fstream.h>
#include <string.h>
#include <elliptic.h>    // Elliptic Curve Class
...
int main(int argc,char **argv)
{
    ...
    int good[100],l[200],t[100],sl[8],k,tau,lambda;
    ...
    while (ip<argc)
    {
        ...
        if (!gotI && strcmp(argv[ip],"-i")==0)
        {
            ...
            if (ip<argc)
            {
                ...
                mueller.open(argv[ip],ios::nocreate|ios::in);
                if (!mueller)
                {
...
//
            ...
            for (r=2;r<=max_r;r++)
            {
                ...
                v=jacobi((Big)k,(Big)lp);     // check Schoof Prop. 6.3
                jj=(lp+1)/r;
...
//
            ...
            BOOL gen;
            while (jacobi((Big)qnr,(Big)lp)!=(-1)) qnr++;
...
//
            ...
            for (jj=1;jj<r;jj++)
            {
                ...
                else if (jacobi((Big)tau,(Big)lp)==1)
                {
                    candidates+=2;
                    tau=sqrmp(tau,lp);
                    tau=(2*tau)%lp;
...
//
        ...
        for (lambda=1;lambda<=(lp-1)/2;lambda++)
        {
            ...
            if (jacobi((Big)k,(Big)lp)!=discrim) continue; 
//
