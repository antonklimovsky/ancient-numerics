...
// See ftp://ftp.compapp.dcu.ie/pub/crypto/README for details
//
...
// obtained from ftp://ftp.compapp.dcu.ie/pub/crypto/schoof.exe 
//
...
{
    ofstream ofile;
    int low,lower,ip,pbits,lp,i,j,jj,m,n,nl,L,t[100],k,tau,lambda;
...
*/
    ...
    int l[100],pp[100];   // primes and powers 
        ...
        if (!anomalous && prime((Big)lp))
        {
            if (degree(Fl)==0) 
            {
                for (tau=0;tau<=lp/2;tau++)
                {
                    ...
                    if (jacobi((Big)jj,(Big)lp)!=(-1)) permisso[tau]=FALSE;
                }
            }
            else
